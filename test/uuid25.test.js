...
describe("Uuid25", function () {
	...
	it("parses prepared cases", function () {
		for (const e of TEST_CASES) {
			...
			assertThrows(() => Uuid25.parseUuid25(e.crockford), SyntaxError);
			...
			assertThrows(() => Uuid25.parseHex(e.crockford), SyntaxError);
			...
			assertThrows(() => Uuid25.parseHyphenated(e.crockford), SyntaxError);
			...
			assertThrows(() => Uuid25.parseBraced(e.crockford), SyntaxError);
			...
			assertThrows(() => Uuid25.parseUrn(e.crockford), SyntaxError);

			assertThrows(() => Uuid25.parseCrockford(e.uuid25), SyntaxError);
			assertThrows(() => Uuid25.parseCrockford(e.hex), SyntaxError);
			assertThrows(() => Uuid25.parseCrockford(e.hyphenated), SyntaxError);
	...
	it("throws error when parsing invalid string", function () {
		...
		for (const e of cases) {
			...
			assertThrows(() => Uuid25.parseCrockford(e), SyntaxError);
		}

		// Additional invalid Crockford-specific cases
		const crockfordInvalidCases = [
			" 4G4MNE3FFEPQK4AJC3M4Z6KNAP", // leading space
			"4G4MNE3FFEPQK4AJC3M4Z6KNAP ", // trailing space
			"4G4MNE3FFEPQK4AJC3M4Z6KNA", // too short (25 chars)
			"4G4MNE3FFEPQK4AJC3M4Z6KNAPX", // too long (27 chars)
			"4G4MNE3FFEPQK4AJC3M4Z6KNA-", // invalid character (-)
			"4G4MNE3FFEPQK4AJC3M4Z6KNA@", // invalid character (@)
		];

		for (const e of crockfordInvalidCases) {
			assertThrows(() => Uuid25.parseCrockford(e), SyntaxError);
		}

		// Overflow case throws Error (not SyntaxError)
		assertThrows(
	...
	it("parses Crockford with character substitutions", function () {
		...
		const uppercase = "4G4MNE3FFEPQK4AJC3M4Z6KNAP";
		const lowercase = "4g4mne3ffepqk4ajc3m4z6knap";
		const mixed = "4G4mne3ffEPQk4AJc3m4z6KNAP";

		const p1 = Uuid25.parseCrockford(uppercase);
		const p2 = Uuid25.parseCrockford(lowercase);
		const p3 = Uuid25.parseCrockford(mixed);

		assert(p1.value === p2.value);
		assert(p2.value === p3.value);
		assert(p1.value === "8j7qcpk2yebp9ouobnujfc312");

		// Test O/o -> 0 substitution
		const with0 = "0SRRVHFQBRJ1ZHAFE2T4N3AZNV";
		const withO = "OSRRVHFQBRJ1ZHAFE2T4N3AZNV";
		const witho = "osrrvhfqbrj1zhafe2t4n3aznv";

		const p4 = Uuid25.parseCrockford(with0);
		const p5 = Uuid25.parseCrockford(withO);
		const p6 = Uuid25.parseCrockford(witho);

		assert(p4.value === p5.value);
		assert(p5.value === p6.value);
		assert(p4.value === "1ixkdgkqeu8wln1vfrw6csla3");

		// Test I/i -> 1 substitution
		const with1_I = "0SRRVHFQBRJIZHAFE2T4N3AZNV";
		const with1_i = "0srrvhfqbrjizhafe2t4n3aznv";

		const p7 = Uuid25.parseCrockford(with1_I);
		const p8 = Uuid25.parseCrockford(with1_i);

		assert(p7.value === p4.value);
		assert(p8.value === p4.value);

		// Test L/l -> 1 substitution
		const with1_L = "0SRRVHFQBRJLZHAFE2T4N3AZNV";
		const with1_l = "0srrvhfqbrjlzhafe2t4n3aznv";

		const p9 = Uuid25.parseCrockford(with1_L);
		const p10 = Uuid25.parseCrockford(with1_l);

		assert(p9.value === p4.value);
		assert(p10.value === p4.value);

		// Test all substitutions combined
		const allSubst = "OsrrvhfqbrjIzhafe2t4n3aznv"; // O->0, I->1
		const p11 = Uuid25.parseCrockford(allSubst);
		assert(p11.value === p4.value);

		// Test that output is always uppercase
		const encoded = p1.toCrockford();
